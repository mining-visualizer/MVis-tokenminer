...
// this has been extended to preserve blank lines and comments (full line only)
// method: blank lines and comments are added as special key/value pairs, where the key is set to ";"
...
namespace boost { namespace property_tree { namespace ini_parser
{
    ...
    void read_ini_ex(std::basic_istream<typename Ptree::key_type::value_type> &stream, Ptree &pt)
    {
        ...
        while (stream.good())
        {
            ...
            line = property_tree::detail::trim(fullLine, stream.getloc());

			if (!line.empty() && line[0] == lbracket)
			{
				// new section
			...
			else
			{
				...
				else if (line.empty() || line[0] == semicolon || line[0] == hash)
				{
					// comment or empty line
					container.push_back(std::make_pair(";", Ptree(fullLine)));
				}
				else
				{
     ...
     */
    template<class Ptree>
    void read_ini_ex(const std::string &filename, 
                  Ptree &pt,
    ...
    namespace detail
    {
        ...
        void check_dupes(const Ptree &pt)
        {
            ...
            lastkey = &it->first;
            for(++it; it != end; ++it) {
				// key = ";" represents empty line or comment
				if (*lastkey == it->first && *lastkey != ";")
                    BOOST_PROPERTY_TREE_THROW(ini_parser_error(
                        "duplicate key", "", 0));
                lastkey = &it->first;
            }
                   ...
                   int flags = 0)
    {
             ...
             it != end; ++it)
        {
            ...
            if (it->second.empty()) {
				//  key == ";" represents empty line  or comment
				if (it->first == ";")
					stream << it->second.template get_value< std::basic_string<Ch> >() << Ch('\n');
				else
					stream << it->first << Ch('=') << it->second.template get_value< std::basic_string<Ch> >() << Ch('\n');
			} else {
                ...
                for (typename Ptree::const_iterator it2 = it->second.begin(), end2 = it->second.end(); it2 != end2; ++it2)
                {
					...
					if (it2->first == ";")
						stream << it2->second.template get_value< std::basic_string<Ch> >() << Ch('\n');
					else
