...
class EMA
{
	// calculate the exponential moving average of a series of data points
	//
	// Definition:
	//     alpha = 2 / (1 + Period)
	//     EMA = alpha * CurrentVal + (1 - alpha) * LastEMA
	//       or (just a re-worked form of the equation)
	//     EMA = (CurrentVal - LastEMA) * alpha + LastEMA

	// lower values for period will result in the EMA following the raw data
	// curve more tightly (new values are given more weight, old values less).

public:

	EMA(int _period) : m_period(_period), m_firstVal(true), m_value(0) {}

	double newVal(double _dataPoint)
	{
		m_value = m_firstVal ? _dataPoint : (_dataPoint - m_value) * 2.0 / (1.0 + m_period) + m_value;
		if (m_firstVal && m_value != 0)
			m_firstVal = false;
		return m_value;
	}
